...
export const robloxConnectionSchema = z.discriminatedUnion("targetType", [
  z.object({
    ...
    robloxUserId: z.string().regex(/^\d+$/, "Roblox User ID must be numeric"),
    apiKey: z.string().min(10).max(512),
  }),
  z.object({
    targetType: z.literal("GROUP"),
    robloxGroupId: z.string().regex(/^\d+$/, "Roblox Group ID must be numeric"),
    apiKey: z.string().min(10).max(512),
  }),
]);
...
export function validateAudioFile(file: { type: string; size: number }) {
  ...
  if (!ALLOWED_AUDIO_MIME_TYPES.has(file.type)) {
    errors.push("Format file tidak didukung. Gunakan MP3, WAV, atau OGG.");
  }
